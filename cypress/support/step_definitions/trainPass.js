...
//   cy.get(button[id=':r1:-list-option-5']).click();
});
...
When("user clicks on Miami from dropdown", () => {
    cy.get('button[id=":r1:-list-option-5"]').click();
});
...
When("user clicks on West Palm Beach", () => {
    cy.get('button[id=":r2:-list-option-1"]').click();
});
