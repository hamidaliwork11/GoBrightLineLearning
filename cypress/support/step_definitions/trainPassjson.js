...
When("user select values from fixture", () => {
    ...
    cy.fixture("ticket_d").then((data) => {
        ...
        cy.get("[role=option]").contains(data.travel_from).click();
        cy.get("[name=travelTo]").click();

        // cy.get("[name=travelTo]").should('be.visible');

        cy.get("[role=option]").contains(data.travel_to).click();
    ...
    cy.get('.blte-btn blte-btn--color-white blte-btn--size-medium blte-btn--width-full blte-btn--variant-primary blte-train-passes-categories__button-item','Premium').click();
        ...
        Then('the total cost should be {string}', (myCost) => {
            ...
            cy.wait(5000);
            cy.get('[class="blte-text blte-passes-cost-summary-item__price blte-passes-cost-summary-item__price--isTotal"]').should('contain',myCost) 
         });
